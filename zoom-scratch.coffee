...
module.exports = (robot) ->
  ...
  robot.respond /zoom-test me( .+)?$/i, (res) ->
    ...
    is_recorded = /rec/i.test res.match[1]
        ...
        .post() (error, response, body) ->
          switch response.statusCode
            when 200
              ...
              if json_body.error?
                res.send "zoom error: #{json_body.error.message}"
              else
                ...
                res.send "#{username} started a #{descriptor} zoom session: #{join_url}"
            else
              res.send "zoom? more like doom! there was a problem sending the request :("
    catch e then res.send e
